							...
							<div style="text-align:right;margin-bottom:10px;">
								...
								<select id="type_id" class="no-shadow">
									...
									<c:forEach items="${typeList}" var="typeInfo" varStatus="status" >
										<option value="${typeInfo.id}" <c:if test="${typeInfo.id==typeId}">selected</c:if>>${typeInfo.typeName}</option>
									</c:forEach>
								</select>
								<!-- 标题检索 -->
								<input type="text" class="text" id="keyWord" name="keyWord" value="${keyWord}" placeholder="检索文章标题" />
								<!-- 点击查询按钮 -->
...
<script>
    ...
    $("#submit").click(function() {
        ...
        if (opt=="move") {
            ...
            $.ajax({
                url : "deleteManyArticleToRecycle.json",
                type : "POST",
                dataType : "json",
                traditional : "true",
                data : {
